...
import { InlinePackage } from './composer.json';
...
function createResponseError(response: AxiosResponse, message?: string): Packagist.ResponseError {
    const error: Packagist.ResponseError = new Error(message ? message : response.statusText) as any
...
export class Packagist {
    ...
    // protected handleInvalidResponse(response: AxiosResponse | AxiosError) {
    ...
    // protected async request(method: AxiosRequestConfig['method'], url: string, config?: AxiosRequestConfig) {
