...
import { ComposerJSON }                        from '../interfaces';
...
export namespace Packagist {
    ...
    export interface Package {
        ...
        versions: Record<string, ComposerJSON.InlinePackage> // [list of versions and their dependencies, the same data of composer.json]
...
export class Packagist {
    ...
    async list(filters?: { vendor?: string, type?: string }):Promise<string[]> {
        const response = await this.client.get('/packages/list.json', { params: filters });
        return response?.data?.packageNames;
    }

    protected resolveVendorNameParams(vendor, name = null) {
        if ( name === null ) {
            name   = vendor.split('/')[ 1 ];
    ...
    async getPackage(vendor, name = null): Promise<Packagist.Package> {
        [ vendor, name ] = this.resolveVendorNameParams(vendor, name);
