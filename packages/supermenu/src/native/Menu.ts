...
import { MenuConfig as BaseMenuConfig} from '../interfaces';
...
export interface MenuConfig extends BaseMenuConfig {
    prefix?: string
    disableIcons?: boolean
    on?: { [ event: string ]: any }
}
...
export class Menu extends MenuNode {
    config: MenuConfig;
    protected classes: WrappedClassList;
    public items: MenuItem[] = [];

    public constructor(public readonly el: HTMLElement, config: Partial<MenuConfig> = {}) {
        super(config);
        this.classes   = wrapClassList(el.classList);
        el[ '__menu' ] = this;
        Array.from(el.children).filter(el => el.classList.contains(`${this.config.prefix}-item`)).forEach(itemEl => {
            new MenuItem(itemEl as any, this, this);
        });
    }

    configure(config: Partial<MenuConfig>) {
        this.config = merge({ prefix: 'sm' }, this.constructor.defaultConfig, config) as MenuConfig;
    }
    ...
    bind() {
        ...
        for ( const eventName in (this!.config!.on || {}) ) {
    ...
    protected handleModifierCall(name: string, value?: boolean) {
        let cls = `${this.config.prefix}--${name}`;
        if ( value !== undefined ) {
...
// @ts-ignore
export class MenuItem extends MenuItemNode {
    ...
    constructor(public readonly el: HTMLElement, public readonly menu: Menu, protected parent: Menu | MenuItem) {
        ...
        let selector = `:scope > .${this.menu.config.prefix}-item__content`;
        let content  = el.querySelector<HTMLElement>(selector);
        selector     = `${selector} > .${this.menu.config.prefix}-item`;
        this.els     = {
            content,
            ...
            submenu     : el.querySelector(`:scope > .${this.menu.config.prefix}-item__submenu`),
            submenuItems: Array.from(el.querySelectorAll(`:scope > .${this.menu.config.prefix}-item__submenu > .${this.menu.config.prefix}-item`)),
        };
    ...
    handleMouseOver  = (event?: MouseEvent) => {
        ...
        this.fire('hover:enter', event);
    };
    handleMouseLeave = (event?: MouseEvent) => {
        this.classes.ensureNot('is-hovered');
        this.fire('hover:leave', event);
    };
